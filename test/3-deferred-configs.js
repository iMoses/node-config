...
vows.describe('Tests for deferred values - JavaScript').addBatch({
  'Configuration file Tests': {
    ...
    'values which are functions remain untouched unless they are instance of DeferredConfig': function() {
      ...
      // console.log(config.get('template);
      // console.log(CONFIG.template.env);
      // console.log(CONFIG.template.file);
      assert.equal(config.get('welcomeEmail.aFunc')(), 'Still just a function.');
    },
    ...
    "defer functions can simply refer to 'this'" : function () {
        assert.equal(config.get('welcomeEmail.justThis'), 'Welcome to this New Instance!');
    },
    ...
    "defer function return original value." : function () {
      assert.equal(config.get('original.original'), 'an original value');
    },

    "second defer function return original value." : function () {
      assert.equal(config.get('original.deferredOriginal'), undefined);
    },

    "defer functions resolved within an array" : function () {
      assert.equal(config.get('list.2'), 3);
      assert.equal(config.get('fromList'), 6);
    },

    "defer functions execution order resolves when accessing other defer functions" : function () {
      assert.equal(config.get('a'), 'my this is 6!');
      assert.equal(config.get('c'), 'my this is 6! this is 6!');
    },
  }
})
.export(module);
