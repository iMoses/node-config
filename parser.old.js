...
var ParserOld = module.exports;

ParserOld.parse = function(filename, content) {
...
ParserOld.xmlParser = function(filename, content) {
  if (!XML) {
    XML = require(XML_DEP);
  }
  var x2js = new XML();
...
ParserOld.jsParser = function(filename, content) {
  return require(filename);
};

ParserOld.tsParser = function(filename, content) {
...
ParserOld.coffeeParser = function(filename, content) {
...
ParserOld.icedParser = function(filename, content) {
...
ParserOld.yamlParser = function(filename, content) {
  ...
  if (Yaml) {
    ...
    content += '\n';
    return VisionmediaYaml.eval(ParserOld.stripYamlComments(content));
  }
  else {
...
ParserOld.jsonParser = function(filename, content) {
  try {
...
ParserOld.json5Parser = function(filename, content) {
  if (!JSON5) {
    JSON5 = require(JSON5_DEP);
  }
  return JSON5.parse(content);
...
ParserOld.hjsonParser = function(filename, content) {
  if (!HJSON) {
...
ParserOld.tomlParser = function(filename, content) {
  if(!TOML) {
...
ParserOld.csonParser = function(filename, content) {
  if (!CSON) {
    CSON = require(CSON_DEP);
  }
  // Allow comments in CSON files
  if (typeof CSON.parseSync === 'function') {
    return CSON.parseSync(ParserOld.stripComments(content));
  }
  return CSON.parse(ParserOld.stripComments(content));
};

ParserOld.propertiesParser = function(filename, content) {
  if (!PPARSER) {
 ...
 */
ParserOld.stripComments = function(fileStr, stringRegex) {
 ...
 * @return {string} The string with comments stripped.
 */
ParserOld.stripYamlComments = function(fileStr) {
...
var definitions = {
  coffee: ParserOld.coffeeParser,
  cson: ParserOld.csonParser,
  hjson: ParserOld.hjsonParser,
  iced: ParserOld.icedParser,
  js: ParserOld.jsParser,
  json: ParserOld.jsonParser,
  json5: ParserOld.json5Parser,
  properties: ParserOld.propertiesParser,
  toml: ParserOld.tomlParser,
  ts: ParserOld.tsParser,
  xml: ParserOld.xmlParser,
  yaml: ParserOld.yamlParser,
  yml: ParserOld.yamlParser,
};

ParserOld.getParser = function(name) {
  return definitions[name];
};

ParserOld.setParser = function(name, parser) {
...
ParserOld.getFilesOrder = function(name) {
  if (name) {
...
ParserOld.setFilesOrder = function(name, newIndex) {
