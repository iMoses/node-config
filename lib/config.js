...
module.exports.parser.validators[0] = Parser.lib.validators
  .gitCrypt(!utils.getOption('CONFIG_SKIP_GITCRYPT'));
 ...
 *     strict: NODE_CONFIG_STRICT_MODE,
 *     freeze: !ALLOW_CONFIG_MUTATIONS,
 *     legacy: true,
 * })
 *
 ...
 * strict {boolean}     determines whether autoload validations are strict
 *                      defaults: NODE_CONFIG_STRICT_MODE
 * freeze {boolean}     determines whether an immutable config will be produced
 *                      defaults: !ALLOW_CONFIG_MUTATIONS
 * legacy {boolean}     determines whether to load legacy supported configuration from:
 *                      - custom-environment-variables.*
 *                      - EnvVar NODE_CONFIG
 *                      - CliArg NODE_CONFIG
 *                      If set to false these will not be loaded
 *                      defaults: false (true on default instance)
 *
 ...
 *
 * @param moduleName    name of the module, used by sub-modules
 * @param options
 * @constructor
 * @property config     lazily loads an locks the configuration object
 * @property parser     the active parser instance for easy access
 * @property options    immutable version of the options passed to the constructor
 * @property sources    an array containing details about the configuration sources
 * @property whenReady  lazily loads to resolve async configurations,
 *                      returns a Promise.all which waits until all async configurations has been resolved
 */
function Config(moduleName, options) {
  ...
  const keepAutoload = fn => ([ autoload ] = [autoload, fn()])[1];
  ...
  this.extend = (object, source = 'config.extend') => {
    if (object) {
      if (moduleName) {
        ...
        sources.push(utils.deepFreeze({module, source, data}));
      }
      else {
        sources.push(utils.deepFreeze({source, data: utils.cloneDeep(object)}));
      }
   ...
   * @return {Config}
   */
  this.parseFile = (filename, middleware) => {
    Array.isArray(middleware) || (middleware = []);
    const content = parser.parse(filename, middleware);
    return Array.isArray(content)
      ? content.forEach(([ source, data ]) => this.extend(data, source))
      : this.extend(content, filename);
  };
  ...
  /**
   * Registers module's config according to its package.json
   *
   * @param moduleDir
   * @return {*}
   */
  this.registerModule = moduleDir =>
    keepAutoload(() => {
      ...
      const configDir = Path.resolve(moduleDir, 'config');
      const parseFile = moduleDir !== process.cwd()
        ? filename => this.parseFile(filename, [
          (filename, content) => ({[pkg.name]: content[pkg.name]})
        ])
        : this.parseFile;
      const options = utils.extendDeep({}, this.options, {configDir});
      this.collectConfigFiles(options).forEach(parseFile);
      return this;
    });
  ...
  utils.attachPropertyValue(this, 'options', Object.freeze(utils.cloneDeep(options)));
...
/**
 * Autoload configuration object from default options and custom variables
 *
 * This method reproduces the same default actions previous versions maintained
 *
 * @returns {Config}
 */
Config.prototype.loadDefaults = function(legacy = false) {
  ...
  if (legacy) {
    ...
    this.parser.collectFiles(this.options.configDir, legacyFiles)
      .forEach(filename => this.parseFile(filename, [
        this.parser.lib.middleware.customEnvironmentVariables,
      ]));
...
Config.prototype.collectConfigFiles = function(options) {
  ...
  return this.parser.collectFiles(configDir, allowedFiles);
};

/**
 * Returns options for the default config instance built from env-vars and cli-args
 *
 * We clear keys of undefined values to make sure destructuring defaults will
 * be applied, which isn't the case as long as the key exists no matter the value
 *
 * @return {*}
 * @see utils.getOption
 */
...
function resolveDeferred(config) {
  utils.collect(config, val => val instanceof DeferredConfig)
    .map(([ deferred, key, object ]) => deferred.prepare(config, object, key))
 ...
 * @param options
 */
function strictValidations(sources, options) {
